...
function CartDrawer({ cart, total, settings, onClose, onChangeQty, onRemove, onOrdered }) {
  ...
  const waMessage = buildMessage();
  const waUrl = `https://api.whatsapp.com/send?phone=${settings.whatsapp}&text=${encodeURIComponent(waMessage)}`;

  function handleSendClick() {
    onOrdered({ ...form, items: cart, total, date: new Date().toISOString() });
    onClose();
  }
        ...
        {step === "cart" ? (
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <a
              ...
              onClick={form.name && form.phone ? handleSendClick : (e) => e.preventDefault()}
              className="block text-center w-full py-3 rounded font-medium mt-2"
              style={form.name && form.phone
                ? { background: "#C9A227", color: "#12100E" }
                : { background: "#2A241C", color: "#6B6355", pointerEvents: "none" }}
            >
              Envoyer la commande sur WhatsApp
            </a>
            <p className="text-[10px] text-center text-[#6B6355] pt-1">
              Le bouton ne s'ouvre pas ? Appuyez sur "..." en haut de l'écran puis "Ouvrir dans le navigateur".
            </p>
          </div>
        )}
      </div>
